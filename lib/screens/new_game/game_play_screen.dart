...
class _GamePlayScreenState extends State<GamePlayScreen>
  ...
  void _showCharacterSheet(CharacterOption character) {
    showDialog<void>(
      ...
      builder: (context) => CharacterSheetDialog(
        ...
        unlockedBadgeLevels: Map<String, int>.from(gameRound.unlockedBadgeLevels),
      ),
    );
  }
  ...
  void _handleSpinResult(String result) {
    ...
    if (result == 'fail') {
      ...
      // Check if numeric result >= current level for WIN
      print(
        'Comparing: $resultLevel >= $currentLevel = ${resultLevel >= currentLevel}',
      );
      ...
      if (resultLevel >= currentLevel) {
        ...
        setState(() {
          ...
          _showResultFeedback = true;
          // Increase eco crisis level for next round
          gameRound.updateEcoCrisisLevel(currentLevel + 1);
        });
        ...
        _showGameResultDialog(
          ...
          isSuccess: true,
          // After closing WIN dialog → open character sheet so player sees badge
          onClosed: () {
  ...
  Widget _buildActionCardsStrip() {
    ...
    final cardWidgets = _displayedActionCards.map((assetPath) {
      return GestureDetector(
        onTap: () => _showImagePopup(assetPath),
        child: Padding(
    ...
    return Padding(
      ...
      child: Container(
        height: isCompactHeight ? 136.0 : 192.0, // scale based on cardHeight + padding
